...
function option:SetText(Text)
    ...
    option.Size = UDim2.new(1, -12, 0, textService:GetTextSize(option.main.Text, 15, Enum.Font.Code, Vector2.new(option.main.AbsoluteSize.X, 9e9)).Y + 6)
end
    ...
    if binding then
        ...
        if (input.KeyCode.Name == option.key or input.UserInputType.Name == option.key) and not binding then
            if option.mode == "toggle" then
                ...
                library.flags[option.flag] = true
                if Loop then Loop:Disconnect() option.callback(true, 0, option.key) end
...
elseif syn then
    pcall(function() gethui(self.base) end)
...
library.SettingsMain:AddBind({text = "Panic Key",nomouse = true, callback = library.options["Unload Cheat"].callback})
...
library.SettingsMenu:AddList({text = 'Background', flag = 'UI Background', values = {'Floral', 'Flowers', 'Circles', 'Hearts'}, value = "Floral", callback = function(Value)
...
if library.Main then
    ...
    library.main.TileSize = UDim2.new(0, 90, 0, 90)
end

-- Credits
library.CreditSection = library.SettingsColumn1:AddSection('Credits');
library.CreditSection:AddLabel('Mikee = Coder');
library.CreditSection:AddLabel('Nah = Cool');